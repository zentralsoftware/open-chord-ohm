...
public final class SocketProxy extends Proxy implements Runnable {
	...
	static void shutDownAll() {
		...
		SocketProxy[] sockets = new SocketProxy[proxies.size()];
		Collection<SocketProxy> collection = proxies.values();
		SocketProxy[] arr = collection.toArray(sockets);
		for (int i=0;i<arr.length;i++)
		{
			SocketProxy proxy = arr[i];
			proxy.disconnect();
		}
		proxies.clear();
	}
